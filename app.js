...
request(url, function(error, response, html) {
  ...
  function getPoses(url) {
    pagePromises.push(new Promise((suc) => {
      request(url, function(error, response, html) {
        $('.post-item .caption a').each(function () {
          var href = $(this).attr('href');

          posePromises.push(new Promise((suc) => {
            request(href, (error, response, html) => {
              var $ = cheerio.load(html);

              var fields = {
                img : $('.tiled-gallery-item img').attr('data-orig-file')
              };

              $('.acro-meta-fields tr').each(function () {
                fields[$(this).find('.label').text()] = $(this).find('.data').text();
              });
